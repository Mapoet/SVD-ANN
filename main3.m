...
for i=1:ifacenum
    for j=1:iperfacenum-ifacetrain
        for k=1:ifacenum
            Fronum(i,j,k)=norm(double(projecttest{i,j})-double(perfaceproject{k,1}),'fro');
        end
    end
end
...
for i=1:ifacenum
    ...
    Xtrain(1:svdk,1:ifacenum*ifaceuse)=0;
    Xid(1,1:ifacenum*ifaceuse)=0;
    Xtest(1:svdk,1:ifacenum*(iperfacenum-ifacetrain))=0;
    Xid2(1,1:ifacenum*(iperfacenum-ifacetrain))=0;
for i=1:ifacenum
    for j=1:ifaceuse
        [fa,fd]=eig(perfaceproject{i,j});
       Xtrain(:,(i-1)*ifaceuse+j)=angle(fd(fd~=0));
       Xid(1,(i-1)*ifaceuse+j)=i;
    end
    for j=1:iperfacenum-ifacetrain
        [fa,fd]=eig(projecttest{i,j});
        Xtest(:,(i-1)*(iperfacenum-ifacetrain)+j)=angle(fd(fd~=0)); 
